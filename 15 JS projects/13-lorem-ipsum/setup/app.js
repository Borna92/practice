...
const loremDisplay = document.querySelector('.lorem-text')
const numOfParagraphs = document.querySelector('#amount')


button.addEventListener('click', (event) =>{
  loremDisplay.innerHTML = ''
  event.preventDefault()
  for(let i = 0; i < numOfParagraphs.value; i++){
    let randomNumber = Math.floor(Math.random()*text.length)
    loremDisplay.innerHTML += `<p>${text[randomNumber]}</p>`
  }
})
